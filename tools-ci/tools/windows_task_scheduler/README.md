...
```bat
C:\example\simple-deploy\tools-ci\tools\windows_task_scheduler\run_pipeline.cmd
```
...
schtasks /Create ^
  ...
  /SC DAILY ^
  /ST 08:00 ^
  /TR "C:\example\simple-deploy\tools-ci\tools\windows_task_scheduler\run_pipeline.cmd" ^
  /RL HIGHEST ^
  /F
```
